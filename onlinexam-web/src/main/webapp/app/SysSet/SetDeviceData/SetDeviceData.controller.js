(function() {
    ...
    angular
        .module('webfullApp')
        .controller('SetDeviceDataController', SetDeviceDataController);

    SetDeviceDataController.$inject = ['$scope', '$timeout', '$stateParams', '$resource', '$uibModal', '$filter', 'DeviceDataService'];

    function SetDeviceDataController($scope, $timeout, $stateParams, $resource, $uibModal, $filter, DeviceDataService) {
        var vm = this;
        vm.jstree = {
            selectedNodeId: '',
            selectedLevel: ''
        ...
        $scope.open = function(describe, name, paramType, limitLow, limitHigh) {
            ...
            var modalInstance = $uibModal.open({
                ...
                templateUrl: 'app/SysSet/SetDeviceData/SetDeviceData.modal.html', //模态框的页面内容,这里的url是可以自己定义的,也就意味着什么都可以写
                controller: 'SetDeviceDataModalCtrl', //这是模态框的控制器,是用来控制模态框的
                controllerAs: '$ctrl',
                size: 'lg', //模态框的大小尺寸
                scope: $scope,
                keyboard: false,
